...
// Hiển thị form checkout (GET /checkout)
Route::get('/checkout', [CheckoutController::class, 'show'])
     ->name('checkout.show');

// AJAX sinh QR + nội dung chuyển khoản (POST /checkout/bank-ref)
Route::post('/checkout/bank-ref', [CheckoutController::class, 'ajaxBankRef'])
     ->name('checkout.bankRef');

// Xử lý lưu đơn COD hoặc Bank (POST /checkout/confirm)
...
|--------------------------------------------------------------------------
|
| Các route /admin/* chỉ cho user đã auth + is_admin (bạn có thể thêm
| middleware kiểm quyền tương ứng)
|
*/

Route::prefix('admin')
->middleware(['auth'])->name('admin.')->group(function(){
